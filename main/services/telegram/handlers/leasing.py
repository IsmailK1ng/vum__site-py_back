...
_COLLATERAL = {
    'ru': 'Условие: <b>В залог имущества</b>',
    'uz': "Shart: <b>Garov evaziga</b>",
    'en': 'Condition: <b>Against collateral</b>',
}
...
_TEXT = {
    ...
    'choose_platform':  {'ru': 'Выберите платформу:',            'uz': 'Platformani tanlang:',          'en': 'Choose platform:'},
    ...
    'no_products':      {'ru': 'Техники с ценами нет. Обратитесь к менеджеру.', 'uz': "Narxli texnika yo'q. Menejer bilan bog'laning.", 'en': 'No priced products. Please contact a manager.'},
    ...
    'session_expired':  {'ru': 'Сессия устарела. Начните расчёт заново.', 'uz': 'Sessiya eskirdi. Qaytadan hisoblang.', 'en': 'Session expired. Please recalculate.'},
...
@sync_to_async
def _get_products_by_filter(language: str, category: str | None = None, subcategory: str | None = None) -> list[dict]:
...
def _calculate(price: int, down_pct: int, term: int) -> dict:
    ...
    if total == 0:
        return {
            'price': price, 'down_payment': 0, 'down_pct': 0,
            'financing': 0, 'monthly_payment': 0, 'total_payment': 0,
            'annual_rate': annual_rate, 'term': term,
        }
...
def _build_down_kb(lang: str) -> InlineKeyboardMarkup:
    ...
    row1 = [InlineKeyboardButton(text=f'{p}%', callback_data=f'lz_down:{p}') for p in [30, 40]]
    row2 = [InlineKeyboardButton(text=f'{p}%', callback_data=f'lz_down:{p}') for p in [50, 60]]
    return InlineKeyboardMarkup(inline_keyboard=[
        row0, row1, row2,
        [InlineKeyboardButton(
            text=_TEXT['back_list'][lang],
...
def _build_result_kb(lang: str, site_url: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            text=_TEXT['apply'][lang],
            callback_data='lz_apply',
        )],
        [
            InlineKeyboardButton(
                text=_TEXT['change_params'][lang],
                callback_data='lz_back:down',
            ),
            InlineKeyboardButton(
                text=_TEXT['details'][lang],
                url=f'{site_url}/lizing/',
            ),
        ],
        [InlineKeyboardButton(
            text=_TEXT['new_calc'][lang],
            callback_data='lz_back:browse',
        )],
    ])
...
def _format_product_card(product: dict, index: int, total: int, lang: str) -> str:
    ...
    if product.get('year'):
        year_label = {'ru': 'Год', 'uz': 'Yil', 'en': 'Year'}.get(lang, 'Yil')
    ...
    if product.get('features'):
        feat_label = {'ru': 'Комплектация', 'uz': 'Komplektatsiya', 'en': 'Equipment'}.get(lang, 'Komplektatsiya')
        lines.append('')
...
def _format_result(calc: dict, title: str, lang: str) -> str:
    ...
    collateral = f"\n{_COLLATERAL[lang]}" if calc['down_pct'] == 0 else ''
...
async def _show_product_card(
    ...
    photo = None
    if image_path := product.get('image_path'):
        try:
...
async def _send_result(
    ...
    """Генерирует PNG и отправляет: сначала картинка, потом текст с кнопками."""
    result_text = _format_result(calc, title, lang)
    result_kb   = _build_result_kb(lang, site_url)
    ...
    await callback.message.answer(result_text, reply_markup=result_kb)
...
async def handle_lz_browse(callback: CallbackQuery, state: FSMContext, user: TelegramUser | None):
    ...
    if mode == 'by_type':
        ...
        if not categories:
            await callback.answer(_TEXT['no_products'][lang], show_alert=True)
            return
        await state.set_state(LeasingStates.choose_category)
        await callback.message.edit_text(
            _TEXT['choose_category'][lang],
...
async def handle_lz_category(callback: CallbackQuery, state: FSMContext, user: TelegramUser | None):
    data     = await state.get_data()
    lang     = data.get('language', 'ru')
...
async def handle_lz_subcategory(callback: CallbackQuery, state: FSMContext, user: TelegramUser | None):
    data        = await state.get_data()
    lang        = data.get('language', 'ru')
...
async def handle_lz_nav(callback: CallbackQuery, state: FSMContext, user: TelegramUser | None):
    data     = await state.get_data()
    lang     = data.get('language', 'ru')
...
async def handle_lz_select(callback: CallbackQuery, state: FSMContext, user: TelegramUser | None):
    data     = await state.get_data()
    lang     = data.get('language', 'ru')
    products = data.get('products', [])
    index    = int(callback.data.split(':')[1])
...
async def handle_lz_down(callback: CallbackQuery, state: FSMContext, user: TelegramUser | None):
    data     = await state.get_data()
    lang     = data.get('language', 'ru')
...
async def handle_lz_term(callback: CallbackQuery, state: FSMContext, user: TelegramUser | None):
    data     = await state.get_data()
    lang     = data.get('language', 'ru')
    ...
    if not price or down_pct is None:
        await callback.answer(_TEXT['session_expired'][lang], show_alert=True)
        await state.clear()
        return
    ...
    await callback.answer()
    await _send_result(callback, calc, data.get('selected_product_title', ''), lang, site_url)


@router.callback_query(F.data.startswith('lz_back:'))
async def handle_lz_back(callback: CallbackQuery, state: FSMContext, user: TelegramUser | None):
    data   = await state.get_data()
    lang   = data.get('language', 'ru')
    ...
    if target == 'browse':
        await state.set_state(LeasingStates.choose_browse_mode)
        try:
            await callback.message.edit_text(_TEXT['enter'][lang], reply_markup=_build_browse_kb(lang))
        except Exception:
            await callback.message.answer(_TEXT['enter'][lang], reply_markup=_build_browse_kb(lang))
...
async def handle_lz_apply(callback: CallbackQuery, state: FSMContext, user: TelegramUser | None):
    data = await state.get_data()
    lang = data.get('language', 'ru')
    calc = data.get('calc', {})

    if not user or not user.phone:
        await callback.answer(_TEXT['no_phone'][lang], show_alert=True)
        return
    ...
    if lead:
        await callback.message.answer(_TEXT['lead_success'][lang], reply_markup=main_menu)
        logger.info('Leasing lead: telegram_id=%s lead_id=%s', callback.from_user.id, lead.id)
    else:
        await callback.message.answer(_TEXT['lead_error'][lang], reply_markup=main_menu)
