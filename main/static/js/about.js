...
if (!testimonialsSlider) {
  ...
  function setupTestimonialsAutoplayControl() {
    const testimonialsCards = document.querySelectorAll('.mxd-testimonials-card');
    ...
    testimonialsCards.forEach(card => {
      ...
      // Остановка автоплея при клике на карточку (навсегда до клика вне)
    ...
    // Возобновление автоплея при клике вне карточек
    document.addEventListener('click', function (e) {
      const clickedCard = e.target.closest('.mxd-testimonials-card');
  ...
  // Инициализация контроля автоплея
  setupTestimonialsAutoplayControl();
};
// --------------------------------------------- //
...
// --------------------------------------------- //
// YouTube Video Control for Swiper Autoplay Start
// --------------------------------------------- //

function onYouTubeIframeAPIReady() {
  const video = document.getElementById('youtube-video-1');
  let swiper = window.swiperInstances ? window.swiperInstances['.swiper-testimonials-2'] : null;

  if (!swiper && document.querySelector('.swiper-testimonials-2')) {
    swiper = new Swiper('.swiper-testimonials-2', {
      slidesPerView: 'auto',
      grabCursor: true,
      spaceBetween: 30,
      autoplay: {
    ...
    window.swiperInstances['.swiper-testimonials-2'] = swiper;
  }
...
// --------------------------------------------- //
// YouTube Video Control for Swiper Autoplay End
// --------------------------------------------- //
// --------------------------------------------- //
...
const initMarquees = () => {
  ...
  if (items) {
    ...
    items.forEach((itemBlock) => {
      ...
      let timeScaleClamp = gsap.utils.clamp(1, 3);
      ScrollTrigger.create({
        start: 0,
        end: "max",
        onUpdate: (self) => {
...
function initFAWCounters() {
  ...
  // Counter 2: 500+ Happy Customers  
  if (document.getElementById('stats-counter-2')) {
    const counter2 = new countUp.CountUp('stats-counter-2', 800, {
      suffix: '+',
      duration: 2.5,
      enableScrollSpy: true,
      scrollSpyOnce: true
  ...
  if (document.getElementById('stats-counter-3')) {
    const counter3 = new countUp.CountUp('stats-counter-3', 75, {
      suffix: '+',
      duration: 2.5,
      enableScrollSpy: true,
      scrollSpyOnce: true
...
function initCountersFallback() {
  // ПРАВИЛЬНЫЕ ЗНАЧЕНИЯ ИЗ ВАШЕГО КОДА
  const counters = [
    { id: 'stats-counter-1', value: '15 млн+' },
    { id: 'stats-counter-2', value: '800+' },
    { id: 'stats-counter-3', value: '75+' },
    { id: 'stats-counter-4', value: '200+' }
  ];
