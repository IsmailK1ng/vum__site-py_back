...
function pageAppearance() {
  ...
  gsap.to(fadeInItems, { duration: 0.8, ease: 'none', opacity: 1, delay: 0.5 });
}
// --------------------------------------------- //
...
$(function() {
  ...
  $(".mxd-nav__wrap").each(function() {
    ...
    // Close the menu when any menu link is clicked — use existing openMenu to keep animation/teardown consistent
    menuLinkEl.on("click", function () {
      openMenu(false);
    });
    // Also delegate to any anchor inside the menu wrapper (covers markup where links use .main-menu__link)
    menuWrapEl.on('click', 'a', function (e) {
      // let natural navigation happen; just trigger menu close
      openMenu(false);
    });
